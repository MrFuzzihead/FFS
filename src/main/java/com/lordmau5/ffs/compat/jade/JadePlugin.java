...
public class JadePlugin implements IWailaPlugin {
    ...
    public void registerClient(IWailaClientRegistration registration) {
        registration.addTooltipCollectedCallback((iTooltip, accessor) -> {
            ...
            iTooltip.getTooltip().add(1, helper.progress(progress, textComponent, progressStyle, BoxStyle.getNestedBox(), true));

            IElement icon = helper.item(new ItemStack(FFSItems.tit.get()), 1.0f).size(new Vec2(16, 16)).translate(new Vec2(-4, -1));

            iTooltip.getTooltip().add(1, icon);
            ...
            iTooltip.getTooltip().append(1, text);
        });
    }
